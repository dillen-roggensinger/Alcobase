...
class Inputer{
	...
	function insertSold_At($input){
		...
		if($input['store_name']=="null" || !$val->valid($input['store_name'],1,100)){	//Can't be null
		...
		. "' and s.price=" . $input['price'] . " and s.did=".$input['did'] . " and s.quantity="
		. $input['quantity'];
	 ...
	 * Takes an array of length 6 that has:
	 * (username, location, store_name, did, quantity, price)
	 */
	function insertBought($input){
		...
		if($input['store_name']=="null" || $val->valid($input['store_name'])){	//Can't be null
			echo("Invalid store_name!<br>");
			return false;
		}
		if($input['location']=="null" || $val->valid($input['location'])){	//Can't be null
			echo("Invalid store_name!<br>");
			return false;
		}
		...
		$query="
		SELECT a.username
		FROM account a, sold_at s
		WHERE s.did=" . $input['did'] . "and s.quantity=" . $input['quantity'] .
		" and s.location='" . $input['location'] . "' and s.store_name='" . $input['store_name'] .
		"' and s.price=" . $input['price'] . " and a.username='" . $input['username'] . "'";
		...
		SELECT b.did
		FROM bought b
		WHERE b.username='" . $input['username'] . "' and b.did=".$input['did'] . " and b.price="
		. $input['price'] . " and b.quantity=".$input['quantity'] . " and b.time=sysdate";
		
		echo("Query2:<br>".$query."<br>");
		...
		$query="INSERT INTO bought VALUES('" . $input['username'] . "',sysdate,'" . $input['location'] .
		"','" . $input['store_name'] . "'," . $input['did'] . "," . $input['quantity'] . "," . $input['price'] .")";
		$stid = oci_parse($conn, $query);
		$err=oci_execute($stid);
